...
export function PopularRoutes() {
  const { language, t, tNum, tDistrict, tDuration, tCurrency } = useLanguage();
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-12">
          <div>
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">
              {language === "bn" ? "জনপ্রিয় আন্তঃজেলা " : "Top Inter-District "}
              <span className="gradient-text">{language === "bn" ? "রুটসমূহ" : "Routes"}</span>
            </h2>
          ...
          >
            {language === "bn" ? `সকল ${tNum("500+")} রুট দেখুন` : "View All 500+ Routes"}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {popularRoutes.map((route, idx) => {
            const numericPrice = parseInt(route.price.replace(/[^0-9]/g, "")) || 0;
            return (
              <Link
                key={idx}
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-base font-extrabold text-emerald-700">
                      {tCurrency(numericPrice)}
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="text-xs text-slate-600 font-semibold flex items-center gap-1">
                      <Building2 className="h-3 w-3 text-slate-400" />
                      {tNum(route.operators)} {language === "bn" ? "অপারেটর" : "Operators"}
                    </span>
                  </div>
                </div>
              </Link>
